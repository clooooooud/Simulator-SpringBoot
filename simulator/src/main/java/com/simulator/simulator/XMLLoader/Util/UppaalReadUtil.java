...
public class UppaalReadUtil {
    public static LinkedList<TaskGraph> uppaalTaskReader(){

        LinkedList<TaskGraph> taskGraphList = new LinkedList<>();

        InputStream in = UppaalReadUtil.class.getClassLoader().getResourceAsStream("TaskGraph0903.xml");
...
//                System.out.println(graph.getName());
                ...
                String graphName = (String) graph.attribute("name").getData();
                dataModelList = new LinkedList<>();
                modelTaskList = new LinkedList<>();
                GlobalTaskList = new LinkedList<>();
                Task.id = 0;


                Iterator<Element> iterator = graph.element("task_list").elementIterator();
...
//                int priority =  Integer.parseInt((String)properties.attribute("priority").getData());
                    ...
                    for(int i = 0;i < instCnt;i++){
                        int job_inst_idx = i;
                        Task task1 = new Task(taskName, knrlType, instCnt, cost, priority,job_inst_idx,graphId);
                        TaskList.add(task1);
                    }

                    //任务输入输出
                    Element proc_items = task.element("proc_items");
                    Iterator<Element> proc_itemsIterator = null;
                    if(proc_items != null)proc_itemsIterator= proc_items.elementIterator();
                    /**
                     * 一个输入输出数据
                     * <proc_item data_name="Data0" id="0" mov_dir="1">
                     *     一个Data实例
...
//                System.out.println(22);
                ...
                Iterator<Element> dataIterator = null;
                if(data_info != null) {
                    dataIterator = data_info.elementIterator();
                }
